  ...
  async function saveAppData() {
    ...
    try {
      const payload = {
        ...
        app_nome: appForm.app_nome.trim(),
        mac_address: appForm.mac_address.trim(),
        app_key: appForm.app_key.trim() || null,
      };

      if (appForm.id) {
        const { error } = await supabase.from('dispositivos').update(payload).eq('id', appForm.id);
        if (error) throw error;
      } else {
  ...
  return (
    <div className="p-4 md:p-8 max-w-[1200px] mx-auto space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <h1 className="text-3xl font-black tracking-tighter uppercase flex items-center gap-2"><Users className="text-primary" />Clientes</h1>
        <div className="grid grid-cols-2 gap-2 w-full md:flex md:w-auto">
          <div className="relative col-span-2 w-full md:w-72"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground h-4 w-4" /><Input placeholder="Buscar por nome..." className="pl-10 bg-card border-border rounded-xl h-11" value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }} /></div>
          <select className="h-11 min-w-0 rounded-xl border border-border bg-card px-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring md:w-44" value={planFilter} onChange={(e) => { setPlanFilter(e.target.value); setCurrentPage(1); }}><option value="">Todos os planos</option>{data?.filterPlans?.map((plan: Option) => <option key={plan.id} value={plan.id}>{plan.name}</option>)}</select>
          <select className="h-11 min-w-0 rounded-xl border border-border bg-card px-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring md:w-44" value={serverFilter} onChange={(e) => { setServerFilter(e.target.value); setCurrentPage(1); }}><option value="">Todos os servidores</option>{data?.filterServers?.map((server: Option) => <option key={server.id} value={server.id}>{server.name}</option>)}</select>
        </div>
      </div>
