...
import { Users, Search, ChevronLeft, ChevronRight, MessageCircle, Send, Pencil, ChevronDown, CalendarDays, RefreshCw, Minus, Plus, Smartphone, Copy, PlusCircle, Gift, History, WalletCards } from 'lucide-react';
...
function ClientesPage() {
  ...
  const [historyData, setHistoryData] = useState<{ renewals: any[]; transactions: any[] }>({ renewals: [], transactions: [] });
  ...
  async function openHistory(client: Client) {
    ...
    setHistoryData({ renewals: [], transactions: [] });
    try {
      const [{ data: renewals, error: renewalsError }, { data: transactions, error: transactionsError }] = await Promise.all([
        supabase.from('renovacoes').select('id, valor, desconto, vencimento_anterior, novo_vencimento, data_renovacao, created_at').eq('cliente_id', client.id).order('data_renovacao', { ascending: false }),
        supabase.from('transacoes').select('id, tipo, entrada, custo, valor, lucro_liquido, descricao, data, created_at').eq('cliente_id', client.id).order('created_at', { ascending: false }),
      ]);
      if (renewalsError) throw renewalsError;
      if (transactionsError) throw transactionsError;
      setHistoryData({ renewals: renewals || [], transactions: transactions || [] });
    } catch (error) {
      console.error(error);
  ...
  async function confirmRenew(mode: RenewMode = renewMode) {
    ...
    try {
      ...
      const valorNormal = Math.max(0, Number(plan.price || 0) - Number(selectedClient.desconto || 0));
      const valorEntrada = mode === 'bonus' ? 0 : valorNormal;
      const totalCusto = (serverRows || []).reduce((sum: number, server: any) => sum + Number(server.valor || 0), 0);
      ...
      const descricao = mode === 'bonus' ? `Bônus por indicação — ${selectedClient.nome}` : `Renovação cliente ${selectedClient.id}`;
      const { error: transactionError } = await supabase.from('transacoes').insert({ user_id: userId, cliente_id: selectedClient.id, tipo: mode === 'bonus' ? 'saida' : 'entrada', entrada: valorEntrada, custo: totalCusto, valor: valorEntrada, data: todayBr, descricao, serv_id: selectedClient.servidores_ids?.[0] || null }); if (transactionError) throw transactionError;
      ...
      setSelectedClient({ ...selectedClient, vencimento: renewDate }); setIsRenewOpen(false); setIsRenewSuccessOpen(true); toast.success(mode === 'bonus' ? `Bônus aplicado a ${selectedClient.nome}.` : `${selectedClient.nome} renovado.`); await refetch();
  ...
  const Actions = ({ client, mobile = false }: { client: Client; mobile?: boolean }) => <div className="flex items-center justify-end gap-2">
    <Button size="icon" variant="outline" title="Histórico" aria-label={`Histórico de ${client.nome}`} onClick={() => openHistory(client)} className={`${mobile ? 'h-11 w-11' : 'h-9 w-9'} rounded-xl`}><History size={mobile ? 18 : 16} /></Button>
  ...
  const selectedDevices = data?.clients?.find((client: Client) => client.id === selectedClient?.id)?.devices || selectedClient?.devices || [];
  ...
  return <div className="p-4 md:p-8 max-w-[1200px] mx-auto space-y-6 animate-in fade-in duration-500">
    ...
    <Dialog open={isHistoryOpen} onOpenChange={setIsHistoryOpen}><DialogContent className="max-w-lg rounded-2xl max-h-[88vh] overflow-y-auto"><DialogHeader><DialogTitle className="flex items-center gap-2 text-xl font-black uppercase tracking-tighter"><History className="text-primary" size={20} />Ficha do cliente</DialogTitle><DialogDescription>{selectedClient?.nome}</DialogDescription></DialogHeader>{selectedClient && <div className="space-y-4 py-2"><div className="grid grid-cols-2 gap-2"><div className="rounded-2xl border bg-muted/20 p-3"><span className="text-xs text-muted-foreground">Plano</span><p className="font-bold break-words">{selectedClient.plans?.name || '—'}</p></div><div className="rounded-2xl border bg-muted/20 p-3"><span className="text-xs text-muted-foreground">Vencimento</span><p className="font-bold font-mono">{selectedClient.vencimento ? format(parseISO(selectedClient.vencimento), 'dd/MM/yyyy') : '—'}</p></div><div className="rounded-2xl border bg-muted/20 p-3"><span className="text-xs text-muted-foreground">Servidor</span><div className="pt-1"><ServerBadge name={selectedClient.serverName || 'N/A'} /></div></div><div className="rounded-2xl border bg-muted/20 p-3"><span className="text-xs text-muted-foreground">WhatsApp</span><p className="font-bold break-all">{selectedClient.whatsapp || '—'}</p></div></div>{selectedDevices.length > 0 && <div className="rounded-2xl border p-4 space-y-2"><span className="text-xs text-muted-foreground">Aplicativos</span>{selectedDevices.map((device: any) => <div key={device.id} className="flex items-center justify-between gap-3 text-sm"><span className="font-semibold">{device.app_nome}</span><span className="text-muted-foreground font-mono text-xs truncate">{device.mac_address || 'Sem MAC'}</span></div>)}</div>}<div><div className="flex items-center justify-between mb-2"><h3 className="font-black uppercase text-sm tracking-wide">Histórico</h3><span className="text-xs text-muted-foreground">{historyData.transactions.length} movimentações</span></div>{historyLoading ? <div className="rounded-2xl border p-6 text-center text-sm text-muted-foreground">Carregando histórico...</div> : historyData.transactions.length === 0 ? <div className="rounded-2xl border p-6 text-center text-sm text-muted-foreground">Nenhuma movimentação registrada.</div> : <div className="space-y-2">{historyData.transactions.map((transaction: any) => { const entrada = Number(transaction.entrada || 0); const custo = Number(transaction.custo || 0); const isBonus = String(transaction.descricao || '').toLowerCase().includes('bônus por indicação'); const dateValue = transaction.data || transaction.created_at; return <div key={transaction.id} className="rounded-2xl border p-3 flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2"><span className={`h-2 w-2 rounded-full ${isBonus ? 'bg-amber-500' : entrada > 0 ? 'bg-emerald-500' : 'bg-red-500'}`} /><p className="font-bold text-sm truncate">{isBonus ? 'Bônus por indicação' : entrada > 0 ? 'Renovação' : transaction.descricao || 'Movimentação'}</p></div><p className="text-xs text-muted-foreground mt-1">{dateValue ? format(parseISO(String(dateValue).slice(0, 10)), 'dd/MM/yyyy') : '—'}</p></div><div className="text-right shrink-0"><p className={`font-bold text-sm ${entrada > 0 ? 'text-emerald-500' : 'text-muted-foreground'}`}>{entrada > 0 ? `+ ${money(entrada)}` : 'R$ 0,00'}</p>{custo > 0 && <p className="text-xs text-red-500">- {money(custo)} custo</p>}</div></div>; })}</div>}</div><Button variant="outline" onClick={() => setIsHistoryOpen(false)} className="w-full h-11 rounded-xl">Fechar</Button></div>}</DialogContent></Dialog>
    ...
    <Dialog open={isRenewOpen} onOpenChange={setIsRenewOpen}><DialogContent className="max-w-sm rounded-2xl"><DialogHeader><DialogTitle className="text-xl font-black uppercase tracking-tighter">{selectedClient?.nome}</DialogTitle><DialogDescription>Ajuste a nova data e escolha o tipo de renovação.</DialogDescription></DialogHeader><div className="grid grid-cols-[52px_1fr_52px] gap-2 py-4"><Button variant="outline" onClick={() => setRenewDate((date) => addDaysISO(date, -1))} className="h-12 rounded-xl"><Minus size={18} /></Button><div className="flex h-12 items-center justify-center rounded-xl border bg-muted/30 font-mono font-bold">{renewDate ? format(parseISO(renewDate), 'dd/MM/yyyy') : ''}</div><Button variant="outline" onClick={() => setRenewDate((date) => addDaysISO(date, 1))} className="h-12 rounded-xl"><Plus size={18} /></Button></div><div className="space-y-2"><Button disabled={isRenewing} onClick={() => { setRenewMode('normal'); void confirmRenew('normal'); }} className="w-full h-11 rounded-xl"><RefreshCw size={16} className="mr-2" />{isRenewing ? 'Renovando...' : 'Renovação normal'}</Button><Button variant="outline" disabled={isRenewing} onClick={() => { setRenewMode('bonus'); void confirmRenew('bonus'); }} className="w-full h-11 rounded-xl border-amber-500/40 text-amber-500 hover:bg-amber-500/10 hover:text-amber-500"><Gift size={16} className="mr-2" />Bônus por indicação</Button><Button variant="ghost" disabled={isRenewing} onClick={() => setIsRenewOpen(false)} className="w-full h-10 rounded-xl">Cancelar</Button></div></DialogContent></Dialog>
