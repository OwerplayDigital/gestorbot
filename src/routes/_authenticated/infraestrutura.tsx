  ...
  return <div className="w-full max-w-full space-y-5 px-4 py-5 sm:px-6 lg:space-y-6 lg:px-8">
    ...
    {loading ? <div className="py-10 text-center text-muted-foreground">Carregando...</div> : tab === 'servidores' ? (
      <section className="space-y-3">
        ...
        {servidores.length === 0 ? <div className="rounded-xl border p-6 text-center text-muted-foreground">Nenhum servidor cadastrado.</div> : servidores.map(s => <div key={s.id} className="rounded-xl border p-4"><div className="min-w-0"><div className="flex items-start justify-between gap-3"><p className="break-words font-medium">{s.name}</p></div><p className="mt-1 text-sm text-muted-foreground">Custo: {money(s.valor)}</p></div><div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => openServer(s)} className="rounded-lg border px-3 py-2 text-sm">Editar</button><button type="button" onClick={() => toggleServer(s)} className="rounded-lg border px-3 py-2 text-sm">{s.active === false ? 'Ativar' : 'Desativar'}</button></div></div>)}
      </section>
    ) : (
      <section className="space-y-3">
        ...
        {planos.length === 0 ? <div className="rounded-xl border p-6 text-center text-muted-foreground">Nenhum plano cadastrado.</div> : planos.map(p => <div key={p.id} className="rounded-xl border p-4"><div className="min-w-0"><div className="flex items-start justify-between gap-3"><p className="break-words font-medium">{p.name}</p></div><p className="mt-1 text-sm text-muted-foreground">Valor padrão: {money(p.price)}</p></div><div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => openPlan(p)} className="rounded-lg border px-3 py-2 text-sm">Editar</button><button type="button" onClick={() => togglePlan(p)} className="rounded-lg border px-3 py-2 text-sm">{p.active === false ? 'Ativar' : 'Desativar'}</button></div></div>)}
      </section>
    )}
