...
import { Users, Clock, AlertCircle, ChevronRight, ArrowRight, History, Activity, Trash2 } from "lucide-react";
...
function Dashboard() {
    ...
    const { data: stats, isLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            try {
                ...
                const [clientsRes, transactionsRes, serversRes, expiringTodayRes, vencidosRes] = await Promise.all([
    ...
    const filteredExpiring = useMemo(() => {
        if (!stats?.expiringToday)
            return [];
        return stats.expiringToday.filter(c => (c.nome || "").toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    return (<div className="flex flex-col gap-6 p-4 pb-12 max-w-lg mx-auto md:max-w-4xl">
        ...
        <Dialog>
          ...
          <DialogContent className="max-h-[80vh] overflow-y-auto">
            ...
            <div className="flex flex-col gap-2 mt-4">
              {stats?.vencidos && stats.vencidos.length === 0 ? (<p className="text-center text-muted-foreground py-8">Nenhum inadimplente encontrado.</p>) : (stats?.vencidos.map(c => (<div key={c.id} className="flex items-center justify-between p-3 border rounded-xl">
      ...
      <section className="space-y-6 pt-4 border-t">
        ...
        <div className="bg-card border rounded-3xl p-6">
          ...
          <div className="h-[200px] w-full">
            ...
            if (active && payload && payload.length) {
                ...
                return (<div className="bg-background border rounded-lg p-2 shadow-xl text-xs">
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col gap-2">
            {stats && stats.recentTransactions && stats.recentTransactions.length === 0 ? (<p className="text-center text-muted-foreground py-4 text-sm">Nenhuma transação este mês.</p>) : (stats?.recentTransactions.map((t) => {
            const entrada = Number(t.entrada || 0);
            const saida = Number(t.custo || 0);
            const lucro = Number(t.lucro_liquido || 0);
            const id = t.id;
            return (<div key={id} className="bg-card border rounded-2xl p-3 flex flex-col gap-2 relative">
                      <div className="absolute top-2 right-2">
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                            <Button variant="ghost" size="icon" className="h-7 w-7 text-rose-500 hover:text-rose-600 hover:bg-rose-500/10 rounded-full" disabled={isDeleting === id}>
                              <Trash2 size={14}/>
                            </Button>
                          </AlertDialogTrigger>
                          <AlertDialogContent>
