    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 pb-12 max-w-7xl mx-auto w-full">
      ...
      <section className="bg-card dark:bg-[#131B2E] border border-border dark:border-slate-800 rounded-2xl p-6 shadow-sm">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h3 className="text-lg font-black tracking-tight text-foreground dark:text-white uppercase leading-none">Performance por Servidor</h3>
          </div>
          <Badge className="bg-primary/10 text-primary border-primary/20 text-[10px] font-black px-2 py-0.5 uppercase">
            {currentMonthLabel}
          </Badge>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
          <div className="relative h-[220px] w-full flex items-center justify-center">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                innerRadius={60}
                  ...
                  dataKey="faturamento"
                >
                  {(stats?.serverStats ?? []).map((entry, index) => (
                    <Cell 
                      key={`cell-${index}`} 
                      fill={['#0284c7', '#22c55e', '#ef4444', '#f59e0b', '#8b5cf6'][index % 5]} 
                      stroke="none"
                    />
                  ))}
                </Pie>
                    ...
                    if (active && payload && payload.length && payload[0]) {
                      ...
                      return (
                        <div className="bg-card dark:bg-[#090D16] border border-border dark:border-slate-800 rounded-xl p-3 shadow-2xl text-xs">
                          <p className="font-black text-foreground dark:text-white mb-2 pb-1 border-b border-border dark:border-slate-800 uppercase">{data.name}</p>
                          <p className="text-primary font-bold">Faturamento: {formatBRL(data.faturamento)}</p>
                        </div>
                      );
                    }
                    return null;
            ...
            <div className="absolute flex flex-col items-center pointer-events-none">
              <span className="text-[10px] font-black text-muted-foreground uppercase leading-none">Total</span>
              <span className="text-sm font-black text-foreground dark:text-white">{formatBRL(stats?.entradas ?? 0)}</span>
            </div>
          </div>

          <div className="space-y-3">
            {stats?.serverStats?.map((server: any, idx: number) => (
              <div key={idx} className="bg-muted/20 dark:bg-slate-900/50 border border-border/50 dark:border-slate-800 rounded-xl p-3 flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full" style={{ backgroundColor: ['#0284c7', '#22c55e', '#ef4444', '#f59e0b', '#8b5cf6'][idx % 5] }} />
                    <span className="text-xs font-black text-foreground dark:text-white uppercase tracking-tight">{server.name}</span>
                  </div>
                  <Badge variant="outline" className="text-[9px] font-black border-border/50">{server.count} Clientes</Badge>
                </div>
                <div className="grid grid-cols-3 gap-2 text-center border-t border-border/10 pt-2 mt-1">
                  <div className="flex flex-col">
                    <span className="text-[8px] font-black text-muted-foreground uppercase">Bruto</span>
                    <span className="text-[10px] font-black text-foreground dark:text-white">{formatBRL(server.faturamento)}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[8px] font-black text-muted-foreground uppercase">Custo</span>
                    <span className="text-[10px] font-black text-rose-500">{formatBRL(server.custo)}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[8px] font-black text-muted-foreground uppercase">Lucro</span>
                    <span className="text-[10px] font-black text-emerald-500">{formatBRL(server.lucro)}</span>
                  </div>
                </div>
              </div>
            ))}
