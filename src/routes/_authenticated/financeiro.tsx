    ...
    if (pageRows.length === 0) {
      ...
      pageRows.forEach((transaction, index) => {
        ...
        const description = truncatePdfText(
          String(transaction.cliente_nome || transaction.nome || transaction.descricao || "Movimentacao"),
          35,
        );
    ...
    <div className="flex w-full max-w-7xl flex-col gap-8 p-4 pb-12 md:p-8">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-[200px] rounded-xl">
              {pastMonths.map((month) => (
                <DropdownMenuItem key={month.value} onClick={() => setSelectedMonth(month)} className="cursor-pointer font-medium">
                  {month.label}
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
          </DropdownMenu>
          <Button variant="outline" size="icon" title="Baixar relatório em PDF" aria-label="Baixar relatório em PDF" className="rounded-xl border-border bg-card shadow-sm" onClick={exportToPDF} disabled={transactions.length === 0}>
            <Download size={18} />
          </Button>
        </div>
      </div>
      ...
      <section className="grid grid-cols-1 gap-6 md:grid-cols-3">
        <Card className="overflow-hidden rounded-2xl border-border bg-card shadow-sm">
          <CardHeader className="pb-2"><div className="flex items-center justify-between"><span className="rounded-lg bg-primary/10 px-2 py-0.5 text-[10px] font-black uppercase tracking-widest text-primary">Faturamento</span><TrendingUp size={16} className="text-primary" /></div></CardHeader>
          <CardContent><div className="text-2xl font-black text-foreground">{formatBRL(stats.entradas)}</div><p className="mt-1 text-[10px] font-bold text-muted-foreground">Total bruto no período</p></CardContent>
        </Card>
        <Card className="overflow-hidden rounded-2xl border-border bg-card shadow-sm">
          <CardHeader className="pb-2"><div className="flex items-center justify-between"><span className="rounded-lg bg-rose-500/10 px-2 py-0.5 text-[10px] font-black uppercase tracking-widest text-rose-500">Custos</span><TrendingDown size={16} className="text-rose-500" /></div></CardHeader>
          <CardContent><div className="text-2xl font-black text-foreground">{formatBRL(stats.saidas)}</div><p className="mt-1 text-[10px] font-bold text-muted-foreground">Total de saídas/servidores</p></CardContent>
        </Card>
        <Card className="overflow-hidden rounded-2xl border border-primary/20 bg-primary/5 shadow-sm">
          <CardHeader className="pb-2"><div className="flex items-center justify-between"><span className="rounded-lg bg-emerald-600/10 px-2 py-0.5 text-[10px] font-black uppercase tracking-widest text-emerald-600">Lucro Líquido</span><DollarSign size={16} className="text-emerald-600" /></div></CardHeader>
          <CardContent><div className="text-2xl font-black text-emerald-600">{formatBRL(stats.lucro)}</div><p className="mt-1 text-[10px] font-bold text-muted-foreground">Resultado final limpo</p></CardContent>
        </Card>
      </section>
    </div>
  );
