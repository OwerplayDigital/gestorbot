...
import { ArrowDownRight, ArrowUpRight, Calendar, ChevronDown, Layers3, Server, WalletCards } from "lucide-react";
...
function serverAccent(name: string) {
  ...
  if (/goat/i.test(name)) return { bar: "bg-orange-500", text: "text-orange-500" };
  if (/p2braz/i.test(name)) return { bar: "bg-purple-500", text: "text-purple-500" };
...
function FinanceiroPage() {
  ...
  const { data: transactions = [], isLoading: loadingFinance } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("transacoes").select("entrada, custo").gte("created_at", startOfMonth(selectedMonth.date).toISOString()).lte("created_at", endOfMonth(selectedMonth.date).toISOString());
  ...
  const { data: analytics, isLoading: loadingAnalytics } = useQuery<AnalyticsData>({
    queryKey: ["financeiro-operational-analytics"], staleTime: 120000,
    queryFn: async () => {
  ...
  const stats = useMemo(() => { const entradas=transactions.reduce((s:number,r:any)=>s+Number(r.entrada||0),0), custos=transactions.reduce((s:number,r:any)=>s+Number(r.custo||0),0); return { entradas, custos, lucro: entradas-custos }; }, [transactions]);
  const brl = (v:number) => v.toLocaleString("pt-BR", { style:"currency", currency:"BRL" });
  const topServer=analytics?.serverRanking[0], topPlan=analytics?.planRanking[0];

  if (loadingFinance || loadingAnalytics) return <div className="mx-auto max-w-7xl p-4 md:p-8"><div className="rounded-2xl border bg-card p-8 text-center text-sm text-muted-foreground">Carregando...</div></div>;
