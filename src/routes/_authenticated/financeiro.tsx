...
import { Calendar, ChevronDown, DollarSign, Download, TrendingDown, TrendingUp } from "lucide-react";
...
function buildFinancialPdf(
  ...
  for (let pageIndex = 0; pageIndex < pages; pageIndex++) {
    ...
    stream += text(margin + 75, headerY + 8, 8, "CLIENTE / DESCRICAO", true, "0.35 0.40 0.50");
    ...
    let rowY = headerY - 25;
    if (pageRows.length === 0) stream += text(margin + 8, rowY - 2, 10, "Nenhuma movimentacao registrada neste periodo.", false, "0.42 0.46 0.54");
    else pageRows.forEach((transaction, index) => {
      if (index % 2 === 1) stream += rect(margin, rowY - 7, pageWidth - margin * 2, 23, "0.985 0.99 1");
      const entrada = Number(transaction.entrada || 0), custo = Number(transaction.custo || 0);
      const description = truncatePdfText(String(transaction.cliente_nome || transaction.nome || transaction.descricao || "Movimentacao"), 35);
  ...
  const catalogId = addObject(""), pagesId = addObject("");
  const fontRegularId = addObject("<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica /Encoding /WinAnsiEncoding >>");
  const fontBoldId = addObject("<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold /Encoding /WinAnsiEncoding >>");
  const pageIds: number[] = [];
  pageStreams.forEach((stream) => {
    const contentId = addObject(`<< /Length ${stream.length} >>\nstream\n${stream}endstream`);
    pageIds.push(addObject(`<< /Type /Page /Parent ${pagesId} 0 R /MediaBox [0 0 ${pageWidth} ${pageHeight}] /Resources << /Font << /F1 ${fontRegularId} 0 R /F2 ${fontBoldId} 0 R >> >> /Contents ${contentId} 0 R >>`));
  });
...
function FinanceiroHistory() {
  ...
  const pastMonths = useMemo(() => {
    ...
    for (let i = 1; i <= 12; i++) {
      ...
      months.push({ label: format(date, "MMMM / yyyy", { locale: ptBR }).replace(/^\w/, (c) => c.toUpperCase()), value: format(date, "yyyy-MM"), date });
    }
    return months;
  }, [nowBr]);
  ...
  const { data: transactions = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const clientNames = new Map((clients || []).map((client: any) => [client.id, client.nome]));
      return rows.map((row) => ({ ...row, cliente_nome: row.cliente_id ? clientNames.get(row.cliente_id) : undefined }));
    },
  });
  const { data: chartData = [] } = useQuery({
    queryKey: ["financeiro-performance-4m"],
    queryFn: async () => {
      const { data, error } = await supabase.from("transacoes").select("data, entrada, custo").order("data", { ascending: true });
      if (error) throw error;
      const rows = data || [];
      const months = ["Jan","Fev","Mar","Abr","Mai","Jun","Jul","Ago","Set","Out","Nov","Dez"];
      const lastFour: { mIdx:number; y:number; label:string }[] = [];
      for (let i=3;i>=0;i--) { const d=subMonths(nowBr,i); lastFour.push({mIdx:d.getMonth(),y:d.getFullYear(),label:months[d.getMonth()]!}); }
      return lastFour.map(({mIdx,y,label}) => {
        const mt=rows.filter((t:any)=>{if(!t.data)return false;const d=parseISO(t.data);return d.getFullYear()===y&&d.getMonth()===mIdx});
        const entradas=mt.reduce((a:number,b:any)=>a+Number(b.entrada??0),0), saidas=mt.reduce((a:number,b:any)=>a+Number(b.custo??0),0);
        return {name:label,entradas,saidas,lucro:entradas-saidas};
      });
    },
  });
  ...
  return <div className="flex w-full max-w-7xl flex-col gap-8 p-4 pb-12 md:p-8">
    <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center"><h1 className="text-2xl font-black uppercase tracking-tighter text-foreground">Histórico Financeiro</h1><div className="flex items-center gap-2"><DropdownMenu><DropdownMenuTrigger asChild><Button variant="outline" className="min-w-[180px] gap-2 rounded-xl border-border bg-card font-bold shadow-sm"><Calendar size={16} className="text-primary" />{selectedMonth.label}<ChevronDown size={16} className="ml-auto text-muted-foreground" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-[200px] rounded-xl">{pastMonths.map((month)=><DropdownMenuItem key={month.value} onClick={()=>setSelectedMonth(month)} className="cursor-pointer font-medium">{month.label}</DropdownMenuItem>)}</DropdownMenuContent></DropdownMenu><Button variant="outline" size="icon" title="Baixar relatório em PDF" aria-label="Baixar relatório em PDF" className="rounded-xl border-border bg-card shadow-sm" onClick={exportToPDF} disabled={!transactions.length}><Download size={18}/></Button></div></div>
    <section className="grid grid-cols-1 gap-6 md:grid-cols-3">
      <Card className="overflow-hidden rounded-2xl border-border bg-card shadow-sm"><CardHeader className="pb-2"><div className="flex items-center justify-between"><span className="rounded-lg bg-primary/10 px-2 py-0.5 text-[10px] font-black uppercase tracking-widest text-primary">Faturamento</span><TrendingUp size={16} className="text-primary" /></div></CardHeader><CardContent><div className="text-2xl font-black text-foreground">{formatBRL(stats.entradas)}</div><p className="mt-1 text-[10px] font-bold text-muted-foreground">Total bruto no período</p></CardContent></Card>
      <Card className="overflow-hidden rounded-2xl border-border bg-card shadow-sm"><CardHeader className="pb-2"><div className="flex items-center justify-between"><span className="rounded-lg bg-rose-500/10 px-2 py-0.5 text-[10px] font-black uppercase tracking-widest text-rose-500">Custos</span><TrendingDown size={16} className="text-rose-500" /></div></CardHeader><CardContent><div className="text-2xl font-black text-foreground">{formatBRL(stats.saidas)}</div><p className="mt-1 text-[10px] font-bold text-muted-foreground">Total de saídas/servidores</p></CardContent></Card>
      <Card className="overflow-hidden rounded-2xl border border-primary/20 bg-primary/5 shadow-sm"><CardHeader className="pb-2"><div className="flex items-center justify-between"><span className="rounded-lg bg-emerald-600/10 px-2 py-0.5 text-[10px] font-black uppercase tracking-widest text-emerald-600">Lucro Líquido</span><DollarSign size={16} className="text-emerald-600" /></div></CardHeader><CardContent><div className="text-2xl font-black text-emerald-600">{formatBRL(stats.lucro)}</div><p className="mt-1 text-[10px] font-bold text-muted-foreground">Resultado final limpo</p></CardContent></Card>
    </section>
    <section className="rounded-2xl border border-border bg-card p-5 shadow-sm"><div className="mb-5"><p className="text-[10px] uppercase tracking-wider font-semibold text-muted-foreground">Movimento</p><h2 className="text-lg font-bold tracking-tight">Performance mensal</h2></div><div className="h-[250px] w-full"><ResponsiveContainer width="100%" height="100%"><BarChart data={chartData} margin={{top:0,right:0,left:-20,bottom:0}} barGap={3} barCategoryGap="16%"><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="currentColor" className="text-border" opacity={.3}/><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fontSize:9,fontWeight:600,fill:"#64748b"}} dy={8}/><YAxis axisLine={false} tickLine={false} tick={{fontSize:9,fontWeight:600,fill:"#64748b"}} tickFormatter={v=>`R$ ${v}`}/><Tooltip cursor={{fill:"rgba(255,255,255,.03)"}} content={({active,payload})=>{if(!active||!payload?.length)return null;const d=(payload[0] as any)?.payload;return <div className="bg-card border border-border rounded-xl p-3 shadow-xl text-xs"><p className="font-bold mb-2">{d.name}</p><p className="text-sky-600 flex justify-between gap-4"><span>Entradas</span><span>{formatBRL(d.entradas)}</span></p><p className="text-rose-500 flex justify-between gap-4"><span>Saídas</span><span>{formatBRL(d.saidas)}</span></p><p className="text-emerald-600 flex justify-between gap-4 border-t border-border pt-1 mt-1"><span>Lucro</span><span>{formatBRL(d.lucro)}</span></p></div>}}/><Legend verticalAlign="top" align="right" height={32} iconType="circle" formatter={v=><span className="text-[9px] font-semibold uppercase tracking-wider text-muted-foreground mr-3">{v}</span>}/><Bar name="Entradas" dataKey="entradas" fill="#0284c7" radius={[5,5,0,0]} maxBarSize={18}/><Bar name="Saídas" dataKey="saidas" fill="#ef4444" radius={[5,5,0,0]} maxBarSize={18}/><Bar name="Lucro" dataKey="lucro" fill="#22c55e" radius={[5,5,0,0]} maxBarSize={18}/></BarChart></ResponsiveContainer></div></section>
  </div>;
}
