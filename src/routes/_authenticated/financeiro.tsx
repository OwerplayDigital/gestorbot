...
function FinanceiroHistory() {
  ...
  const {data:chartData=[]}=useQuery({queryKey:["financeiro-performance-4m"],queryFn:async()=>{const {data,error}=await supabase.from("transacoes").select("data, entrada, custo").order("data",{ascending:true});if(error)throw error;const rows=data||[];const months=["Jan","Fev","Mar","Abr","Mai","Jun","Jul","Ago","Set","Out","Nov","Dez"];const lastFour:{mIdx:number;y:number;label:string}[]=[];for(let i=3;i>=0;i--){const d=subMonths(nowBr,i);lastFour.push({mIdx:d.getMonth(),y:d.getFullYear(),label:months[d.getMonth()]!});}return lastFour.map(({mIdx,y,label})=>{const mt=rows.filter((t:any)=>{if(!t.data)return false;const d=parseISO(t.data);return d.getFullYear()===y&&d.getMonth()===mIdx;});const entradas=mt.reduce((a:number,b:any)=>a+Number(b.entrada??0),0),saidas=mt.reduce((a:number,b:any)=>a+Number(b.custo??0),0);return{name:label,entradas,saidas,lucro:entradas-saidas};});}});
  ...
  return <div className="w-full max-w-7xl p-4 pb-12 md:p-8">
    ...
    <section className="grid gap-4 lg:grid-cols-[1.35fr_.65fr]">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-1">
        <div className="rounded-2xl border bg-card p-4 shadow-sm"><div className="mb-4 flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-500/10 text-emerald-600"><ArrowUpRight size={18}/></div><p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Faturamento</p><p className="mt-1 text-xl font-black">{formatBRL(stats.entradas)}</p></div>
        <div className="rounded-2xl border bg-card p-4 shadow-sm"><div className="mb-4 flex h-9 w-9 items-center justify-center rounded-xl bg-rose-500/10 text-rose-500"><ArrowDownRight size={18}/></div><p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Custos</p><p className="mt-1 text-xl font-black">{formatBRL(stats.saidas)}</p></div>
      </div>
    </section>

    <section className="mt-4 grid gap-4 lg:grid-cols-[1.15fr_.85fr]">
      <div className="rounded-[24px] border bg-card p-5 shadow-sm"><div className="mb-5 flex items-center justify-between"><div><h2 className="font-black tracking-tight">Performance</h2><p className="text-xs text-muted-foreground">Últimos 4 meses</p></div><div className="flex gap-3 text-[10px] font-bold uppercase tracking-wider text-muted-foreground"><span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-sky-600"/>Entradas</span><span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-rose-500"/>Custos</span></div></div><div className="h-[250px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={chartData} margin={{top:5,right:0,left:-20,bottom:0}} barGap={3} barCategoryGap="24%"><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="currentColor" className="text-border" opacity={.35}/><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fontSize:10,fontWeight:600,fill:"#64748b"}} dy={8}/><YAxis axisLine={false} tickLine={false} tick={{fontSize:9,fontWeight:600,fill:"#64748b"}} tickFormatter={(v)=>`R$ ${v}`}/><Tooltip cursor={{fill:"rgba(148,163,184,.06)"}} content={({active,payload})=>{if(!active||!payload?.length)return null;const d=(payload[0] as any)?.payload;return <div className="rounded-xl border bg-card p-3 text-xs shadow-xl"><p className="mb-2 font-bold">{d.name}</p><p className="flex justify-between gap-5 text-sky-600"><span>Entradas</span><b>{formatBRL(d.entradas)}</b></p><p className="flex justify-between gap-5 text-rose-500"><span>Custos</span><b>{formatBRL(d.saidas)}</b></p><p className="mt-1 flex justify-between gap-5 border-t pt-1 text-emerald-600"><span>Lucro</span><b>{formatBRL(d.lucro)}</b></p></div>;}}/><Bar dataKey="entradas" fill="#0284c7" radius={[6,6,2,2]} maxBarSize={22}/><Bar dataKey="saidas" fill="#ef4444" radius={[6,6,2,2]} maxBarSize={22}/></BarChart></ResponsiveContainer></div></div>

      <div className="overflow-hidden rounded-[24px] border bg-card shadow-sm"><div className="flex items-center justify-between border-b px-5 py-4"><div><h2 className="font-black tracking-tight">Movimentações</h2><p className="text-xs text-muted-foreground">{transactions.length} no período</p></div><ReceiptText size={18} className="text-muted-foreground"/></div><div className="divide-y">{latest.length?latest.map((t:any)=>{const entrada=Number(t.entrada||0),custo=Number(t.custo||0);const positive=entrada>0;const value=positive?entrada:custo;return <div key={t.id} className="flex items-center gap-3 px-5 py-3.5"><div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${positive?'bg-emerald-500/10 text-emerald-600':'bg-rose-500/10 text-rose-500'}`}>{positive?<ArrowUpRight size={17}/>:<ArrowDownRight size={17}/>}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{t.cliente_nome||t.nome||t.descricao||'Movimentação'}</p><p className="text-[11px] text-muted-foreground">{t.created_at?format(parseISO(t.created_at),'dd/MM/yyyy'):'-'}</p></div><p className={`shrink-0 text-sm font-black ${positive?'text-emerald-600':'text-rose-500'}`}>{positive?'+':'-'} {formatBRL(value)}</p></div>;}):<div className="px-5 py-10 text-center text-sm text-muted-foreground">Nenhuma movimentação neste período.</div>}</div></div>
    </section>
  </div>;
}
